...
def render_ts(state):
    ...
    if state["station"] is None:
        return html.Div("Just pick one station.")

    try:
        df = read_pfile_from_onedrive(state["source"], state["station"])
        
        if df.empty:
            return html.Div(f"Could not load data for station {state['station']}. Please check OneDrive links.")
